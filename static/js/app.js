...
async function fetchData(url) {
  try {
    ...
    // Note from Davin: modified code - second argument added for chart
...
// Note from Davin: modified code - second argument for createChart and redefinition of wfreq
function createChart(sample, freq) {
  ...
  const barLayout = {
    ...
    xaxis: {
      ...
      tickfont: { size: 10, color: 'rgb(145, 145, 145)' }, 
      automargin: true,
      titlefont: {
        size: 14, 
        color: 'rgb(145, 145, 145)'
...
// Log the wfreq value before attempting calculations
console.log('wfreq:', wfreq);

// Function to create the gauge pointer path
function gaugePointer(wfreq) {
  
  // Note from Davin: I have no idea why but this particular wfreq was logging as a 10x multipler and as a string - thus the division. 
  // This fixed the angle on the gauge. my guess is somehow wfreq+0 was happening somewhere?
...
function setupDropdownChangeHandler(samples, metadata) {
  d3.select("#selDataset").on("change", function () {
    ...
    // Note from Davin: modified code - second argument added.
